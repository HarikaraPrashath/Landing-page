      ...
      <div className="flex animate-scroll ">
        ...
        {techStack.map((tech, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="whitespace-nowrap font-medium text-lg text-white">{tech.name}</span>
            </div>
          )
        })}
        ...
        {techStack.map((tech, index) => {
          ...
          return (
            <div
              key={`third-${index}`}
              className="flex items-center text-white space-x-3 min-w-max px-8 hover:text-gray-900 transition-colors group"
            >
              <div className={`text-3xl ${tech.color} group-hover:scale-110 transition-transform duration-300`}>
                <IconComponent className="w-8 h-8 text-white" />
              </div>
